    ...
    if (nx >= 0 && ny >= 0 && nx < cols && ny < rows) {
        if (maze[ny][nx] === 0) {
            ...
            if (player.x === exit.x && player.y === exit.y) {
                ...
                window.location.href = "Door 80/index.html"; // Change this to your actual next page
            }
        } else {
...
eggImg.onload = () => {
    exitImg.onload = () => {
        ...
        drawMaze();
        setTimeout(() => {
            // Reload the page after a short delay to ensure images are loaded, but only if the maze is not generated
            if (maze.length === 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (up && down && left && right) {
        ...
        up.addEventListener('touchstart', e => { e.preventDefault(); movePlayer(0, -1); });
        down.addEventListener('touchstart', e => { e.preventDefault(); movePlayer(0, 1); });
        left.addEventListener('touchstart', e => { e.preventDefault(); movePlayer(-1, 0); });
        right.addEventListener('touchstart', e => { e.preventDefault(); movePlayer(1, 0); });
